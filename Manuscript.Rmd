...
```{r}
load("~/MissingThePoint/Data/convergence_diagnostics_test.Rdata")
convergence_diagnostics %>% filter(sim == 1, mech == "MCAR", p ==0.95) %>% ggplot(aes(x=it, y = ac.max.qhat)) + geom_line()
convergence_diagnostics %>% filter(sim == 1, mech == "MCAR", p ==0.95) %>% ggplot(aes(x=it, y = ac.max.lambda)) + geom_line()
```
