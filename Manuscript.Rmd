...
With this simulation, we assess the impact of non-convergence on the validity of scientific estimates obtained using the imputation package `{mice}` [@mice]. Inferential validity is reached when estimates are both unbiased and have nominal coverage across simulation repetitions ($n_{sim} = 1000$). To assess non-convergence, we terminate the imputation algorithm after a varying number of iterations ($n_{it} = 1, 2, ..., 50$). We differentiate between nine different missingness scenarios that are defined in the data generating mechanism. 

## Data generating mechanism

We generate one complete set of $n_{obs} = 1000$ cases in each simulation repetition. This set consists of three multivariately normal random variables, such that
...
The complete set is 'amputed' (i.e., made incomplete) according to nine missingness conditions. We use a $3 \times 3$ factorial design with three missingness mechanisms and three proportions of incomplete cases, see Figure XYZ. Missingness mechanisms refer to the probability of being missing for any given entry in a dataset. There are three distinct types of mechanisms, as defined by Rubin (1976). Roughly translated, the probability of being missing may be equal for all entries (MCAR; Missing Completely At Random), the probability of being missing depends on observed information (MAR; Missing At Random), or the probability depends on *un*observed information, making the missingness non-ignorable (MNAR; Missing Not At Random). [add left-tailed M(N)AR mechanism, md pattern, and maybe refer to ampute paper or "Dance of..." by Schouten 2018].

```{r figure-missingness, echo=FALSE}
tibble(prop = c(25,50,75), MCAR = 1:3, MAR = 4:6, MNAR = 7:9) %>% 
  pivot_longer(cols = c(MCAR, MAR, MNAR)) %>% 
  mutate(prop = factor(prop, levels = c(25, 50, 75), labels = c("25%", "50%", "75%")),
         mech = factor(name, levels = c("MCAR", "MAR", "MNAR")),
         text = paste(mech, "missingness\n", prop, "incomplete")) %>% 
  ggplot(aes(x = mech, y = prop, fill = as.factor(value), label = text)) +
  geom_tile(color = "black") +
  geom_text() +
  scale_fill_brewer(palette = "RdYlBu", direction = -1, guide = "none") +
  scale_y_discrete(limits = c("75%", "50%", "25%")) +
  scale_x_discrete(breaks = c("MCAR", "MAR", "MNAR")) +
  theme_minimal() +
  labs(x = "Missingness mechanism", y = "Proportion of incomplete cases")
```
...
We impute the missing data five times ($m = 5$) using Bayesian linear regression imputation with `mice` [@mice]. On each imputed dataset, we perform multiple linear regression as our analysis of scientific interest. We predict outcome variable $Y$ from the other two variables,
...
As recommended by van Buuren, our performance measures are bias, confidence interval width, and coverage rate of the estimands (-@buur18 $\S$ 2.5.2). These measures are averaged across the simulation repetitions.
