...
impute_once <- function(iteration, ...) {
  if(is.null(mids)){
    mids <<- mice(boys, m = n_imp, maxit = 1, seed = 123, printFlag = FALSE)
  }
  else {
...
mids <- mice(boys, m = n_imp, maxit = n_it, seed = 123, printFlag = FALSE)
...
# plot
ggplot(res, aes(x = it, y = est)) +
  geom_line()
ggplot(res, aes(x = it, y = ac.max.mu.hgt)) +
  geom_line()
ggplot(res, aes(x = it, y = ac.max.qhat)) +
  geom_line()
ggplot(res, aes(x = it, y = ac.max.lambda)) +
  geom_line()
ggplot(res, aes(x = it, y = r.hat.max.mu.hgt)) +
  geom_line()
ggplot(res, aes(x = it, y = r.hat.max.qhat)) +
  geom_line()
ggplot(res, aes(x = it, y = r.hat.max.lambda)) +
  geom_line()
